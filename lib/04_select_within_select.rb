...
def richer_than_england
  ...
  SELECT name
  FROM countries
  WHERE gdp/population  >= (SELECT gdp/population AS gdp1 FROM countries WHERE name = 'United Kingdom') AND name != 'United Kingdom' AND continent = 'Europe'
  SQL
end
...
def neighbors_of_certain_b_countries
  ...
  FROM countries
  WHERE continent IN (SELECT continent FROM countries WHERE name IN ('Belize', 
  'Belgium'))
  SQL
end
...
def population_constraint
  ...
  FROM countries
  WHERE population > (SELECT population FROM countries WHERE name = 'Canada') 
